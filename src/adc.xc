 ...
 *      Author: sethu
 *      Example for usage of 2 ADCs on startKIT
 */
...
#pragma select handler                          //Special function to allow select on inuint
...
static void init_adc_periph(chanend c) { //Configures the ADC peripheral for this application
     ...
     for (int i = 0; i < 6; i++) { //Do initial trigger. Do 6 to fill channel buffers
       time += ADC_TRIG_DELAY;
